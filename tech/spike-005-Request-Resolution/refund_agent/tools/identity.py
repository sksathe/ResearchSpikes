...
class IdentityVerifier:
    ...
    async def send_otp(
        ...
        Args:
            customer_id: Customer ID (accepts both CUST-001 and CUST001 formats)
            method: "email" or "sms" (currently only email is supported via Resend)
        
        Returns:
            Dict with OTP status and expiration time
        """
        # Normalize customer_id (remove hyphens) to handle both formats
        normalized_customer_id = customer_id.replace("-", "")
        
        # Check both normalized and original formats
        customer = None
        if normalized_customer_id in _customer_db:
            customer = _customer_db[normalized_customer_id]
        elif customer_id in _customer_db:
            customer = _customer_db[customer_id]
        ...
        # Store OTP using normalized customer_id for consistency
        _otp_storage[normalized_customer_id] = {
            "code": otp_code,
    ...
    async def verify_otp(
        ...
        """
        # Normalize customer_id (remove hyphens) to handle both CUST-001 and CUST001 formats
        normalized_customer_id = customer_id.replace("-", "")
        
        # Check both normalized and original formats
        otp_data = None
        storage_key = None
        if normalized_customer_id in _otp_storage:
            storage_key = normalized_customer_id
            otp_data = _otp_storage[normalized_customer_id]
        elif customer_id in _otp_storage:
            storage_key = customer_id
        ...
        return {
            ...
            "customer_id": normalized_customer_id  # Return normalized format for consistency
        }
